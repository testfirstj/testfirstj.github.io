...
Another security mechanism that the JVM has is the security manager.
The security manager controls whether a specific operation is permitted
or not. 

Operations checked by the security manager include:
...
For a complete look of default permissions that are available have
a look at the 
https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/security/Permission.html[javadoc].

By default Java runs without a security manager installed. This means
that by default all operations are permitted.

You can set a Security Manager with the following code
...
However without specifying permissions somewhere the Security Manager is quite
...
* The `java.policy` file in the Java platform's home directory
* The `.java.policy` file in the user's home directory
* By setting the `java.security.policy` property +
`System.setProperty("java.security.policy", "<path_to_policy>/security.policy");`
* By starting the JVM with the `java.security.policy` argument +
`java -Djava.security.policy==<path_to_policy>/security.policy Application`

The policy file contains a sequence of grant entries, where each entry has
the following form:
...
For example if we want our application to be able to read and write a file
called `test.txt` in the user's home directory and be able to read all
the files in the log directory, we could do that as follows:
```
grant
{
...
Now if the application tries to open a different file, the security
manager will trow a exception
```
java.security.AccessControlException: access denied ("java.io.FilePermission" "<path_to_file>/test2.txt" "read")
```

// File/socket/etc permissions
// Signed code
// Custom permissions
...
// Can be used as certificate for web application
...
On the left we have successful login and on the right a unsuccessful.

.Login page normal usage
[.clearfix]
--
[.left]
.Successful login for user `testuser`
image::security_successful_login.png[Successful login, width=400"]
[.right]
.Unsuccessful login
image::security_unsuccessful_login.png[Unsuccessful login, width=400]
--
