...
icon:linux[fw] `$ sha256sum /path/to/file` +
...
[source,sh]
----
choco install adoptopenjdk17 --params="/ADDLOCAL=FeatureMain,FeatureEnvironment,FeatureJarFileRunWith,FeatureJavaHome /INSTALLDIR=C:\usr\lib\jvm\ /quiet"
----
...
[source,sh]
----
export JAVA_HOME=/usr/lib/jvm/jdk-17.0.15+6/ #<1>
----
...
To install it, do (example uses version 3.9.11  as of 2025-09-22)
...
cd /usr/share
sudo rm -fr maven
sudo tar xf ~/Downloads/apache-maven-3.9.11-bin.tar.gz
sudo ln -sf apache-maven-3.9.11/ maven
cd /usr/bin
...
🐧$ mvn --version
Apache Maven 3.9.11 (a0360b3cef57eeee480750d869b7f9d98cc2a333)
Maven home: /usr/share/maven
Java version: 17.0.15, vendor: Eclipse Adoptium, runtime: /usr/lib/jvm/jdk-17.0.15+6
...
----
cd /usr/local
sudo unzip ~/Downloads/netbeans-27-bin.zip
sudo mv netbeans{,-27}
----
...
----
#!/bin/bash
/usr/local/netbeans-27/bin/netbeans "$@" & #<1>
----
