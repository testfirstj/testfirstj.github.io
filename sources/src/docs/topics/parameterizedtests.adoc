...
To make that work, we sometimes have to [grey]_comment out_ code in the test or in the business part of the code,
because that code is not yet ready (the student has not written it yet).
In such cases, we comment out using two slashes, which is both best and easiest. +
If you want to un-comment such part, select the lines and press kbd:[Ctrl+/] pronounced _control slash_.+
For Mac fans, it's different, of course. kbd:[{Command}+/] pronounce _Command slash_.
====
'''
...
things like reflection, which we may only do in later parts of the course. The trick is to use a `Map` that maps a string to the desired object.
...
----
 static Map<String, Person> emap = Map.of(
            "piet", new Person("Piet", "Puk", LocalDate.parse("1955-03-18"),"M"),
            "piet2", new Person("Piet", "Puk", LocalDate.parse("1955-03-18"),"M"), // for equals test.
...
<1> Note that we use a https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/function/BiFunction.html[BiFunction<T,U,R>],
with *T*, *U*, and *R* all of the same type: Fraction. This is legal.
<2> _ is not used in the right hand side of the arrow. This is legal too.
...
You can specify an external file as a data source to achieve the same; that annotation is `@CsvFileSource`, which takes a file as an argument.
The file can be a CSV, edited with tools like the NetBeans CSV Editor plugin for convenience.
Or with a spreadsheet program like LibreOffice Calc or Microsoft Excel.
...
This means JUnit expects the CSV file to be located at `{project-directory}/src/test/resources`, or within a child folder of `resources` (e.g., `{project-directory}/src/test/resources/data`).
To be able to do that, you *first* need to create a new folder. +
A pro would simply open the terminal in the project directory and do `mkdir -p src/test/resources`, but if you prefer to push the mouse, then in NetBeans you can also do New->Other->Folder, which allows you to select the folder name (resources) and the parent folder (src/test). Take your pick.
====
...
In some cases, multiple tests need the same data. In the case of a CsvSourceFile, that is easily covered: Simply copy the annotations to all places where you need
...
One problem is that a method in Java can return only one result, either an object or a primitive type. Luckily an array is also an object in Java.
There are two solutions to this issue:

1. Create special test data objects of a special test data class, either inside your or outside your test class. It is easy to make a test data class to carry the test data. In this case, make a data-providing method and use the method name in the `@MethodSource` annotation. The test method should understand the test data object.
2. Use the JUnit5-provided `ArgumentsProvider`. This warps an array of objects into a single object, so that all can be returned as a single (array) value in a stream. This eliminates the need for a (simple) test data class.

We will use the latter approach, simply because it is less work for the programmer, either you or me.
